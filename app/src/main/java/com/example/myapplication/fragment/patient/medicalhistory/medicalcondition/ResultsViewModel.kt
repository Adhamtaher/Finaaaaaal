...
class ResultsViewModel @Inject constructor(
    ...
    suspend fun getMedicalRecord(doctorInfo1: DoctorInfo1, token: String) =
        viewModelScope.launch {
            repository.getMedicalRecord(doctorInfo1, token).collect { resource ->
                when (resource) {
                    is Status.Loading -> {
                    ...
                    is Status.Success -> {
                        _loginState.value = loginState.value.copy(
                            ...
                            record = resource.data.records?.get(0)
                        )
